...
void DataChannel::setReply(
  ...
  if (sentSlots_.empty()) {
    if (numFilledSlot_ != batchsize) {
      std::cout << "setReply: numFilledSlots: " << numFilledSlot_
...

  {
    ...
    for (int i = 0; i < (int)slotStatus_.size(); ++i) {
      // assert(slotStatus_[i] == SlotStatus::filled
      //        || slotStatus_[i] == SlotStatus::avail);
...
std::unordered_map<std::string, torch::Tensor> DataChannel::getSlot(
  ...
  cvAvailSlots_.wait(lk, [this] { return availSlots_.size() > 0; });
  int slot = availSlots_.front();
  availSlots_.pop();
...
std::unordered_map<std::string, torch::Tensor> DataChannel::getReply(int slot) {
  ...
  cvReplied_.wait(
      lk, [this, slot] { return slotStatus_[slot] == SlotStatus::replied; });
  lk.unlock();
