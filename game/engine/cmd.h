...
/*  Common commands for all games, specified in cmd.def.
    ...
    Note that all commands will have access to current unit's id, and other
   information provided in the arguments.

    CMD_IMMEDIATE(TacticalMove, PointF, p);
        Move a unit to a target point p.
    CMD_IMMEDIATE(Create, UnitType, build_type, PointF, p, PlayerId, player_id,
        int, resource_used = 0);
        Create a unit at a point p, for player_id and use resource_used.
    CMD_IMMEDIATE(Remove);
        Remove current unit.
    CMD_IMMEDIATE(EmitBullet, UnitId, target, PointF, p, int, att, float,
        speed);
        Emit a bullet at target at point f. Used by ranged attack. When the
        bullet reaches target, it issues a melee attack.
...
class CmdBase {
 ...
 public:
  ...
  explicit CmdBase(Tick t, UnitId iid)
      : CmdBase(iid) {
    _tick = _start_tick = t;
  }
  ...
  Tick start_tick() const {
    return _start_tick;
  }

  void set_tick_and_start_tick(Tick t) {
    _tick = _start_tick = t;
  }
  ...
  void set_id(UnitId id) {
    _id = id;
  }

  void set_cmd_id(int i) {
    _cmd_id = i;
  }

  bool just_started() const {
    return _tick == _start_tick;
  }

  virtual std::unique_ptr<CmdBase> clone() const {
    return std::unique_ptr<CmdBase>(new CmdBase(*this));
  }
  ...
  virtual std::string PrintInfo() const {
    std::stringstream ss;
    ss << "[id:" << _id << "][type:" << type() << "][" << _start_tick << ":"
       << _tick << "]";
    return ss.str();
  }

#define SMALLER_FIRST(field)                                                   \
  if ((c1.field) > (c2.field))                                                 \
    return true;                                                               \
  if ((c1.field) < (c2.field))                                                 \
    return false;
#define LARGER_FIRST(field)                                                    \
  if ((c1.field) < (c2.field))                                                 \
    return true;                                                               \
  if ((c1.field) > (c2.field))                                                 \
    return false;
  ...
  friend bool operator<(const CmdBase& c1, const CmdBase& c2) {
    // since priority_queue will always pop the largest element, we swap the
    // meaning of <. Note that we need to make sure Cmd has total order, since
    // priority_queue is not stable.
    //
    // First make sure most recent command ranked first.
    SMALLER_FIRST(_tick);
    // For the same set of command, make sure the most recent command ranks
    // first.
...
// Durative commands. They do not directly change the game state, but issue
// other immediate commands.
class CmdDurative : public CmdBase {
 protected:
  bool _done;
  virtual bool run(const GameEnv&, CmdReceiver*) {
    return true;
  }

  virtual bool cleanup(const GameEnv&, CmdReceiver*) {
    return true;
  }

 public:
  explicit CmdDurative(UnitId id = INVALID)
      : CmdBase(id)
      , _done(false) {
  }
  explicit CmdDurative(Tick t, UnitId id)
      : CmdBase(t, id)
      , _done(false) {
  }
  explicit CmdDurative(const CmdDurative& c)
      : CmdBase(c)
      , _done(c._done) {
  }

  CmdType type() const override {
    return CMD_DURATIVE;
  }

  // Check whether this command is done. If so, it will be removed from the
  // current queue.
  bool IsDone() const {
    return _done;
  }
  ...
  bool Cleanup(const GameEnv& env, CmdReceiver*);
...
class CmdImmediate : public CmdBase {
 protected:
  virtual bool run(GameEnv*, CmdReceiver*) {
    return true;
  }

 public:
  explicit CmdImmediate(UnitId id = INVALID)
      : CmdBase(id) {
  }
  explicit CmdImmediate(Tick t, UnitId id)
      : CmdBase(t, id) {
  }
  bool Run(GameEnv* env, CmdReceiver* receiver) {
    return run(env, receiver);
  }
...
// Control commands, they are changing game status.
class CmdControl : public CmdBase {
 protected:
  virtual bool run(GameEnv*, CmdReceiver*) {
    return true;
  }
 ...
 public:
  explicit CmdControl(UnitId id = INVALID)
      : CmdBase(id) {
  }
  explicit CmdControl(Tick t, UnitId id)
      : CmdBase(t, id) {
  }
  bool Run(GameEnv* env, CmdReceiver* receiver) {
    return run(env, receiver);
  ...
  SERIALIZER_DERIVED0(CmdControl, CmdBase);
  SERIALIZER_ANCHOR(CmdControl);
  UNIQUE_PTR_COMPARE(CmdControl);
};

using CmdBPtr = std::unique_ptr<CmdBase>;
using CmdDPtr = std::unique_ptr<CmdDurative>;
using CmdIPtr = std::unique_ptr<CmdImmediate>;
using CmdCPtr = std::unique_ptr<CmdControl>;
using AssignedCmds = std::map<UnitId, CmdBPtr>;
